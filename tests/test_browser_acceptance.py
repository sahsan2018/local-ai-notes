...
def test_primary_browser_workflow_history_keyboard_and_phone_viewport(live_app):
    with sync_playwright() as playwright:
        ...
        try:
            ...
            page.reload()
            revision_two = page.locator(".history-row", has_text="Revision 2")
            revision_two.get_by_role("button", name="View").wait_for()
            revision_two.get_by_role("button", name="View").click()
            ...
            before = page.url
            export_link = page.locator("#note-export")
            assert export_link.get_attribute("aria-disabled") == "true"
            export_link.dispatch_event("click")
